...
Read `package.json`, `vite.config.ts`, `wrangler.json`, `app.config.json`, `src/contracts/`, `src/plugin/`, `worker/`, `plugin/`, `scripts/`, and `tests/`. Locate Cloudflare-specific MCP handling, assets, Durable Objects, alarms, storage, and test harnesses. Keep React, Vite, Tailwind, and the MCP Apps bridge.
