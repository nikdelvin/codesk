...
const folders = [
  ...
  { path: 'server/', label: 'Give it capabilities', detail: 'Define MCP tools in stdio.ts and behavior in example.ts. realtime.ts handles live connections.', tag: 'BACKEND', icon: '02' },
    ...
    <div className="mx-auto max-w-6xl px-5 sm:px-8">
      ...
      <header className="flex flex-wrap items-center justify-between gap-5 border-b border-line py-6">
        ...
        <nav aria-label="Main navigation" className="flex flex-wrap items-center gap-x-5 gap-y-3 text-xs font-medium text-muted sm:gap-x-7 sm:text-sm">
          ...
          <a className="hover:text-ink" href="#lifecycle">Saved runs</a>
      ...
      <main id="main">
        <section className="grid items-center gap-12 py-16 lg:grid-cols-[1.05fr_1fr] lg:gap-16 lg:py-24">
          <div>
            ...
            <p className="mt-5 font-mono text-[10px] tracking-wide text-muted">MIT LICENSED <span className="px-2">/</span> YOUR CODE <span className="px-2">/</span> YOUR COMPUTER</p>
          </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-x-7 gap-y-4 border-y border-line py-6 text-sm font-medium">
          ...
          {['React + Vite', 'Tailwind CSS', 'Local SQLite', 'MCP Apps'].map(name => <span key={name}>{name}</span>)}
        </div>
        ...
        <section id="setup" className="grid gap-10 py-20 lg:grid-cols-[0.85fr_1.15fr] lg:gap-20">
          <div>
            ...
            <p className="mt-5 max-w-sm text-sm leading-7 text-muted">Clone your copy of <code className="text-ink">codesk</code>, open a terminal in its local folder, and run setup. It connects the pieces for you.</p>
            <p className="mt-4 max-w-sm text-xs leading-6 text-muted">You’ll need Node.js 24+, Codex Desktop, and cloudflared. Setup installs cloudflared if needed; no account or domain is required. After installation, reopen Codex and start a new task.</p>
          </div>
          <div>
            <div className="overflow-hidden rounded-xl bg-[#172032] text-white shadow-lg shadow-slate-900/5">
              <div className="flex items-center justify-between border-b border-white/10 px-5 py-3">
                <span className="font-mono text-[10px] text-slate-400">TERMINAL · CODESK / LOCAL</span>
            ...
            <ol className="grid grid-cols-2 gap-4 sm:grid-cols-4">
              {['Name it', 'Build locally', 'Check tunnel', 'Install plugin'].map((step, i) => <li key={step} className="border-t border-line pt-3"><span className="eyebrow">0{i + 1}</span><p className="mt-2 text-xs font-medium">{step}</p></li>)}
            </ol>
            <p className="mt-5 text-xs leading-6 text-muted">Setup builds your local runtime, checks its HTTPS/WSS tunnel, then installs the plugin into your Codex profile. No connection files to copy by hand.</p>
          </div>
        </section>
        ...
        <section id="architecture" className="border-t border-line py-20">
          <div className="mb-9 flex flex-wrap items-end justify-between gap-5">
            ...
            <p className="max-w-xs text-sm leading-6 text-muted">One local runtime. Your own SQLite file.<br />Clear places to make your changes.</p>
          </div>
          ...
          <div className="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-dashed border-line px-5 py-4 text-xs leading-6 text-muted">
            ...
            <p><code className="text-ink">app.config.json</code> holds your plugin name.</p>
          </div>
        </section>
        ...
        <section className="grid gap-8 border-t border-line py-12 sm:grid-cols-3" aria-label="Included features">
          {[
            ...
            ['Start small. Make it yours.', 'The counter is a starting point. Replace its tools and UI with your own workflow.'],
        ...
        <section id="lifecycle" className="mb-16 rounded-2xl border border-line bg-surface p-6 sm:p-10">
          <p className="eyebrow mb-4">03 / Ready when you are</p>
          <h2 className="section-title">Your desk remembers.</h2>
          <div className="mt-7 grid gap-7 text-sm leading-7 text-muted sm:grid-cols-3">
            <p>Open the plugin in Codex. Its local runtime and secure tunnel start automatically and stay available while a task or panel is connected.</p>
            <p>Close your tasks and panels. The runtime stops after a short grace period. Your saved runs remain on your computer until you ask to delete them.</p>
            <p>In a new task, ask to list your saved runs and resume one. A new tunnel address needs a fresh panel; your counter and its history stay saved.</p>
          </div>
          <p className="mt-6 text-xs leading-6 text-muted">Application execution and storage are local. The HTTPS/WSS connection passes through Cloudflare, so opening a panel requires an internet connection.</p>
        </section>
      </main>
